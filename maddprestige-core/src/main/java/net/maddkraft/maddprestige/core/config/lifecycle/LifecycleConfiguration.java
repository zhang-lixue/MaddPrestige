...
public record LifecycleConfiguration(
    ...
    public LifecycleConfiguration {
        if (schemaVersion != 4) {
            throw new IllegalArgumentException("Lifecycle schema version must be 4");
        }
        ...
        if (competition.enabled()) {
            throw new IllegalArgumentException("Competition is unsupported and must remain disabled");
        }
    }
    ...
    public static LifecycleConfiguration empty() {
        return new LifecycleConfiguration(4, PrestigeConfiguration.disabled(), Map.of(), Map.of(), Map.of(),
