...
final class RuntimeCommandExecutor implements CommandExecutor, TabCompleter {
    ...
    RuntimeCommandExecutor(Supplier<List<MessageReference>> status, Supplier<List<MessageReference>> providers,
        ...
        List<MessageReference> lines = switch (root) {
            ...
            case "reload" -> {
                if (!sender.hasPermission("maddprestige.admin.config.apply")) {
                    yield List.of(MessageReference.of("command.runtime.permission_denied",
                            "permission", "maddprestige.admin.config.apply"));
                }
                yield reload.get();
            }
            default -> List.of(MessageReference.of("command.runtime.usage"));
        };
