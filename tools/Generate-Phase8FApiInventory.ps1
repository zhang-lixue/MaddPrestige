...
$output = Join-Path $RepositoryRoot 'docs/V2_PHASE8F_API_INVENTORY.md'
...
$paperOutput = Join-Path $RepositoryRoot 'docs/V2_PHASE8F_PAPER_API_INVENTORY.md'
...
$lines.Add('# Phase 8F final Bukkit-free SDK public API inventory and classification')
$lines.Add('')
$lines.Add('Generated mechanically from the compiled `maddprestige-api` classes on 2026-08-24 by `tools/Generate-Phase8FApiInventory.ps1`. This is surface A: the Bukkit-free core/service/provider SDK. Every public member shown by `javap -public` inherits its enclosing type classification. Compiler-standard record/enum members are included, so this is the exact review surface rather than a source-file summary.')
$lines.Add('')
$lines.Add('- Phase 8A public top-level type count: **76**.')
$lines.Add("- Final Phase 8F public top-level type count: **$($types.Count)**.")
foreach ($count in $counts) {
    $lines.Add("- $($count.Name): **$($count.Count)** type(s).")
}
$lines.Add('- Baseline `2.x-stable-1` records the exact 46-type Stable surface. Changes require backwards-compatible extension or an explicitly reviewed later major-version baseline.')
$lines.Add('')
...
$paperLines.Add('# Phase 8F final Paper-specific Stable event API inventory')
$paperLines.Add('')
$paperLines.Add('Generated mechanically from public `@Stable` classes in `maddprestige-platform-paper` and their compiled class files on 2026-08-24 by `tools/Generate-Phase8FApiInventory.ps1`. This is surface B: the Paper-specific event API. It is intentionally separate from the Bukkit-free SDK inventory.')
$paperLines.Add('')
...
$paperLines.Add('- The six events and their accepted Bukkit/Stable-SDK boundary form Paper surface B of baseline `2.x-stable-1`.')
$paperLines.Add('')
...
foreach ($type in $paperTypes) {
    ...
    $paperLines.Add("### ``$($type.Qualified)`` - PAPER STABLE 2.x BASELINE")
    $paperLines.Add('')
    $paperLines.Add('```text')
