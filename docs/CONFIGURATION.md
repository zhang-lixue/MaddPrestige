...
Use `/maddprestige setup ...` for a first simple ladder. Use `/maddprestige config ...` for later expert changes:
...
3. `config set`, `config add`, `config remove`, `config remap`, and `config unmap` edit the draft.
...
missing external group, unsafe stage deletion, absent acknowledgement, or validation error fails closed and leaves the
last known-good revision active.

The exact generic output of setup is in
[`examples/member-adventurer-veteran`](../examples/member-adventurer-veteran). It is executable documentation, not an
alternate activation mechanism.
